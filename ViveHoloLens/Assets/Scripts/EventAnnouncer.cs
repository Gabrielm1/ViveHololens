...
public class EventAnnouncer : MonoBehaviour
{
    ...
    public TicTacToeCtrl TicTacToeController;
    ...
    private void AlignmentClient_EventPlayerAlignmentFinished(bool success)
    {
        if (success)
        {
            Say("You are now aligned. Place the controller in one of the Tic Tac Toe cells and pull the trigger to draw");
        }
        else
        {
            Say("Alignment aborted.");
    ...
    private void TicTacToeController_PlayerWin(string message)
    {
        Say(message + ". You can now restart the game by pressing the menu button");
    }
