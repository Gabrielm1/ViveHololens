...
public class AlignmentManager : NetworkBehaviour
{
    ...
    public GameObject ticTacContainer;
    ...
    public void ApplyLocalAlignment(Vector3 position, float rotation)
    {
        ...
        foreach (var entity in entitiesToAlign)
        {
            ...
            print(entity.name + " | " + entity.transform.position);
        }
    }
    ...
    private void FinishAlignment(Vector3 position, float rotation)
    {
        ...
        Debug.Log("Alignment finished. The pos: " + deltaPosition + " delta rotation: " + deltaRotation);

        CmdInitializeTicTacGrid(deltaPosition, deltaRotation); // Spawn the TicTacToe Grid
    ...
    private void RpcSetBlockParent(GameObject ticTac)
    {
        ticTac.transform.SetParent(ticTacContainer.transform, false);
    }
